...
export async function auditSite(url: string): Promise<AuditResult> {
  ...
  try {
    const resp = await safeFetch(url, {
      headers: { "User-Agent": "CITED-audit/0.1" },
    });
    ...
    if (canonicalHref) {
      try {
        defaults.canonical = new URL(canonicalHref, url).href;
      } catch {
        defaults.canonical = canonicalHref;
      }
    }
  ...
  try {
    const robotsResp = await safeFetch(
      `${origin}/robots.txt`,
      { headers: { "User-Agent": "CITED-audit/0.1" } },
    );
    if (robotsResp.ok) {
  ...
  try {
    const llmsResp = await safeFetch(
      `${origin}/llms.txt`,
      { headers: { "User-Agent": "CITED-audit/0.1" } },
    );
    defaults.has_llms_txt = llmsResp.ok;
  } catch {
    /* leave false */
  }

  try {
    const sitemapResp = await safeFetch(
      `${origin}/sitemap.xml`,
      { headers: { "User-Agent": "CITED-audit/0.1" } },
    );
    if (sitemapResp.ok) {
